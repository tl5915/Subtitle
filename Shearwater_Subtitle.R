shearwater_srt_shift <- function(input_csv, output_srt, shift_seconds) {
  ...
  df <- df[, .SD, .SDcols = c("Time (sec)",
                              "Depth",
                              "Time To Surface (min)",
                              "Water Temp")]
  df[, "time" := as.numeric(get("Time (sec)"))]
  df[, "depth" := round(as.numeric(get("Depth")), 1)]
  df[, "tts" := as.character(floor(as.numeric(get("Time To Surface (min)"))))]
  df[, "temp" := as.integer(round(as.numeric(get("Water Temp"))))]
  ...
  for (i in seq_len(nrow(df))) {
    if (i == 1) {
      ...
      start_time <- format_timestamp(df$time[i] - 5)
      end_time   <- format_timestamp(df$time[i] + 5)
    }
    ...
    srt_content <- paste0(srt_content, "TTS: ", df$tts[i], "min\n")
    srt_content <- paste0(srt_content, "Temp: ", df$temp[i], "°C\n\n")
  }
  ...
  for (i in seq_along(valid_indices)) {
    new_srt_lines <- c(new_srt_lines,
                       as.character(subtitle_counter))
    new_srt_lines <- c(new_srt_lines,
                       paste0(format_timestamp(adjusted_timestamps[i, 1]),
                              " --> ",
                              format_timestamp(adjusted_timestamps[i, 2])))
    subtitle_counter <- subtitle_counter + 1

    start_index <- which(srt_lines == timestamp_lines[valid_indices[i]])[1]
    end_index <- ifelse(i < length(valid_indices),
                        which(srt_lines == timestamp_lines[
                          valid_indices[i + 1]
                        ])[1] - 1,
                        length(srt_lines))
